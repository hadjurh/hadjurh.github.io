...
class MarkdownLoader {
    constructor() {
        this.sections = ['about', 'cv', 'publications', 'phd'];
        this.init();
    }
...
// Hover effect for b/B and bee/Bee/bees/Bees/beehive/Beehive etc.
(function () {
    const WORDS = [
        "Beekeeping", "beekeeping",
        "beehive", "Beehive", "beehives", "Beehives",
        "bees", "Bees",
        "bee", "Bee", 
    ];
    ...
    function applyBHoverEffect(root) {
        ...
        const walker = document.createTreeWalker(
            ...
            NodeFilter.SHOW_TEXT,
            {
                acceptNode(node) {
                    ...
                    if (!/[bB]/.test(v)) return NodeFilter.FILTER_REJECT;
                    ...
                    if (p.classList && p.classList.contains("hover-b")) {
                        return NodeFilter.FILTER_REJECT;
                    }
        ...
        nodes.forEach((textNode) => {
            ...
            let idx = 0;
            outer: while (idx < text.length) {
                for (const w of WORDS) {
                    if (text.startsWith(w, idx)) {
                        frag.appendChild(wrapSpan(w));
                        idx += w.length;
                        continue outer;
                    }
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Built with inspiration from astro-theme-cactus and jrosseruk.github.io');
});
